...
/**
 * Auto-generated EmailLog Routes
 *
...
import type { EmailLogHooks } from "./email-log.hooks"
...
/**
 * Set hooks for EmailLog routes
 */
export function setEmailLogHooks(h: EmailLogHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["to", "subject", "type", "status", "sentAt", "campaignId"] as const
...
/**
 * Create EmailLog CRUD routes
 */
export function createEmailLogRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[EmailLog] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.emailLog.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "EmailLog not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[EmailLog] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.emailLog.create({
        data: picked.data,
      })
      ...
      console.error("[EmailLog] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.emailLog.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[EmailLog] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[EmailLog] Delete error:", error)
