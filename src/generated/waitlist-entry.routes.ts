...
/**
 * Auto-generated WaitlistEntry Routes
 *
...
import type { WaitlistEntryHooks } from "./waitlist-entry.hooks"
...
/**
 * Set hooks for WaitlistEntry routes
 */
export function setWaitlistEntryHooks(h: WaitlistEntryHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["email", "productId", "productTitle", "createdAt"] as const
...
/**
 * Create WaitlistEntry CRUD routes
 */
export function createWaitlistEntryRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[WaitlistEntry] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.waitlistEntry.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "WaitlistEntry not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[WaitlistEntry] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      console.error("[WaitlistEntry] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.waitlistEntry.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[WaitlistEntry] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[WaitlistEntry] Delete error:", error)
