...
/**
 * Auto-generated ProductOverride Routes
 *
...
import type { ProductOverrideHooks } from "./product-override.hooks"
...
let hooks: ProductOverrideHooks = {}
...
/**
 * Set hooks for ProductOverride routes
 */
export function setProductOverrideHooks(h: ProductOverrideHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["shopifyId", "title", "description", "vendor", "productType", "tags", "hidden", "customPrice", "customCompareAt", "updatedAt", "createdAt"] as const
...
/**
 * Create ProductOverride CRUD routes
 */
export function createProductOverrideRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      const [items, total] = await Promise.all([
        ...
        prisma.productOverride.count({ where }),
      ])

      return sendJson(c, { ok: true, items, total })
    } catch (error: any) {
      console.error("[ProductOverride] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.productOverride.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "ProductOverride not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[ProductOverride] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.productOverride.create({
        data: picked.data,
      })
      ...
      console.error("[ProductOverride] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.productOverride.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[ProductOverride] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[ProductOverride] Delete error:", error)
