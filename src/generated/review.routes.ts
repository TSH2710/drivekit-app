...
import type { ReviewHooks } from "./review.hooks"
...
/**
 * Set hooks for Review routes
 */
export function setReviewHooks(h: ReviewHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["productId", "userId", "displayName", "email", "rating", "title", "body", "verified", "createdAt", "updatedAt"] as const
...
export function createReviewRoutes(): Hono {
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "Review not found" } }, 404)
      }
